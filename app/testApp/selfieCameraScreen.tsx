...
export const SelfieCameraScreen: React.FC = () => {
  ...
  // Camera is now clipped to circle, so use circle size for coordinate conversion
  const circleDiameter = CIRCLE_RADIUS * 2;
  const { validationState, validateFace } = useFaceValidation({
    circleBounds: {
      centerX: CIRCLE_RADIUS, // Center of the clipped camera view
      centerY: CIRCLE_RADIUS, // Center of the clipped camera view
      radius: CIRCLE_RADIUS,
    },
    screenWidth: circleDiameter, // Camera view width = circle diameter
    screenHeight: circleDiameter, // Camera view height = circle diameter
  });

  // Get face detector functions
  ...
  // Function to handle face detection results on JS thread
  ...
  // Create worklet function to call JS callback
  const runFaceValidationOnJS = Worklets.createRunOnJS(handleFacesOnJS);

  // Throttled frame processor running face detection at 15 FPS
